...
const add = async function (req, res) {
  ...
  try {
    ...
    if (!newReview)
      return res
        .status(400)
        .send({ message: "Error trying to create review." });

    return res.status(200).send({ message: "Review added successfully." });
  } catch (err) {
    return res
      .status(500)
      .send({ message: "Error trying to add review. Try again." });
  }
};
...
const reviews = async function (req, res) {
  ...
  try {
    reviewsArray = await Review.find({ product: productID }).populate(
      "customer",
    );
    ...
    return res
      .status(500)
      .send({ message: "Error trying to get reviews. Try again." });
  }
};
